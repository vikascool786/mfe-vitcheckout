...
export default function SearchResults(props) {
  ...
  const handleSubFilterClick = (content) => {
    setContextFilterAppliedValue(content);
  ...
  const handleResults = async (e, sortApplied = bestMatchText) => {
  ...
  const handleSelFilterRemove = async (remFil, remId) => {
    ...
    if (remFil.filterType === catalogIdText) {
      removeFilter = {
        ...filterApplied,
        catalogId: filterApplied[catalogIdText].filter((fil) => fil !== remId),
      };
          ...
          <div className="qa-filters filter">
            ...
            {isSortShown && (
              <FilterBox
                ...
                actionIcon={<FontAwesomeIcon icon={faCircle} />}
                handleSeeResults={(e) => handleResults(e, sortApplied)}
              ></FilterBox>
            )}
          </div>
        </>
