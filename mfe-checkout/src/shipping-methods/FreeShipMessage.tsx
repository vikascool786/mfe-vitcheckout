...
export const FreeShipMessage: React.FC<IShippingMessageProps> = ({
  ...
  portalData,
    ...
    let isMAFreeShip = portalData.hasFreeShipping && Boolean(orderStore.store.isMA);
        ...
        doShippingCalc(portalData.portalId, orderStore.items)
            .then(response => {
                if(!orderStore.store.isMA){
                    ...
                    setFreeShipPolicy(buildFreeShipPolicy(response.quotes[catalogId]?.[0]?.storeMessage));
                }
                if(freeShipData.hasFreeShipping){
                    const currency = response.quotes[catalogId]?.[0]?.currency;
                    let freeShipDiff = freeShipData.isMA ? response.quotes[catalogId]?.[0]?.freeShipDiff :
                    ...
                    freeShipData.isFreeShipMet = freeShipData.isMA ? response.quotes[catalogId]?.[0]?.freeShipMet || freeShipDiff <= 0 :
                        freeShipDiff <= 0;
    ...
    useEffect(() => {
        const iframe = document.querySelector(".free-ship-policy-iframe") as HTMLIFrameElement;
        if (iframe) {
            iframe.onload = function () {
                if(iframe && iframe.contentWindow){
                    const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
                    const sectionToKeep = iframeDocument.querySelector('.js-main-content');
