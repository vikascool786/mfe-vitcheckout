...
const Click2PayNewCard = (function () {
  ...
  function buildEncryptCardParams(values) {
    ...
    const parameters = {
      ...
      panExpirationYear: values.cardInfo.expYear.toString().substring(2),
  ...
  function encryptCardFailedHandler(error) {
    ...
    const encryptErrMsg =
      "There has been a problem adding your a new card to your Click to Pay wallet. Please enter a different payment method or try again.";
  ...
  function checkoutWithNewCard(c2pInstance, encryptedCardData) {
    ...
    checkoutPromise
      .then((response) =>
        checkoutWithNewCardSuccessHandler(response, c2pInstance)
      )
  ...
  function refreshCardList(c2pInstance) {
    ...
    cardsPromise
      ...
      .catch((error) => {
        Click2PayUtil.showSpinner(false);
        Click2PayLogger.logInfo(
          "getUserCards() failed message on card refresh: " + error
        );
      });
  }
  ...
  function checkoutWithNewCardFailedHandler(error) {
    const errorMessage = error.message;
    console.error(
      "checkoutWithNewCard() failed error message: " + errorMessage,
      JSON.stringify(error)
    );
    Click2PayLogger.logInfo(
      "checkoutWithNewCard failed message: " + errorMessage
    );
    Click2PayPlaceOrder.closeIFrame();
    const failedMessage =
      "There has been a problem adding your a new card to your Click to Pay wallet. Please try again.";
