...
export const isNewCardInPaymentOptions = (paymentOptions: IPaymentOption[]): boolean => {
...
export const getSelectedPaymentOption = (paymentOptions: IPaymentOption[]): IPaymentOption | undefined => {
    return paymentOptions.find(option => option.isSelected);
}

export const returnPaymentOptionsWithDefaultSelection = (paymentOptions: IPaymentOption[]): IPaymentOption[] => {
    const preferredPaymentMethod = paymentOptions.find(
        (option) => option.paymentMethod.preferred
    );

    const paymentIdToSelect = preferredPaymentMethod ? preferredPaymentMethod.paymentMethod.id : 0;
...
export const isSelectedPaymentInAllowedOrderPayments = (paymentOptions: IPaymentOption[], allowedPaymentOptions: IPaymentMethod2[]): boolean => {
