...
const ProductPrice: React.FC<ProductPriceProps> = ({
  ...
  const productPrice = (
    ...
    if (isSale) {
      ...
      return (
        <>
          <span className="qa-sale-price product-price product-sale-price">{salePrice}</span>
  ...
  const getComputedPrice = useCallback(() => {
    if (permutation && productOption) {
      const selectedPriceValue = productOption.values.find((opt) => {
        return permutation == opt.textValue;
      })?.derived.retailPrice;
      const priceDetails = {
  ...
  return <div className="product-price-container">{getComputedPrice()}</div>;
};
